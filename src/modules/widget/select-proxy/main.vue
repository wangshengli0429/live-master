        ...
        <div class="select-stage-body">
          <div class="search_wrapper">
            ...
            <ul class="stage-list" @scroll="loadMore">
                <template v-if="proxyList && proxyList.length">
                    <li v-for="item in proxyList" class="al" :class="{'active':isActive(item.uuid)}" @click="selectProxy(item)">{{ item.name }}</li>
                </template>
                <template v-else>
                    <li>暂无可选数据</li>
                </template>
...
export default{
  ...
  watch:{
    ...
    getProxyList(start){
          ...
          $API.proxy.getProxyList({orgId:this.orgId,parentId:this.parentId,start,limit:this.limit,searchKey:this.key},resp => {
              this.count = resp.count;
              if(start == 0){
                this.proxyList = resp.list;
              }else{
                this.proxyList = this.proxyList.concat(resp.list);
              }
              this.locked = false;
          })
      },
      ...
      loadMore(e){
        if(e.target.scrollHeight - e.target.scrollTop - e.target.offsetHeight <= 0){
          if(this.proxyList.length < this.count && !this.locked){
            this.lock = false;
            this.getProxyList(this.proxyList.length);
          }
        }
      },
      destroy(){
